...
- Tower parts can contribute single-target tower effects with `tower.singleTarget*` homogeneous values. Each effect independently selects the closest enemy to the tower within that effect's range, then applies one-target push-back, flee, circle, DoT ticks, stun, or infection.
