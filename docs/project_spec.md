...
- City hex background textures live in `src/assets/hexBackgrounds/<type>/<biome>/<vector>` and are cataloged through `src/data/cityHexBackgrounds.ts`. City state stores a biome, maximum city size, generated terrain vector map, per-hex hidden base `background` sprite id/vector, and optional obstacle sprite id/vector. Migration rerolls the biome and terrain map, empty, cleared, and built cells render their base background, unclaimed expansion cells render `removableObstacle` sprites over that base, and cells outside the maximum city radius render `permanentObstacle` sprites. Battle terrain is generated from the same hidden terrain vector map and reuses known base terrain sprite ids, extending it upward beyond the wall so the battlefield reads as a seamless continuation outside the city.
...
The shared upkeep bar uses one flex header row with equal direct slots for People/Gold, Nature, siege status, Tech, and Aether. Missing resource groups leave empty placeholders so slot spacing remains stable, and the center slot shows `SIEGE` only while the city is besieged. The bar also includes a threat meter rendered as the upkeep bar's bottom border. The Nature/Biology indicator renders Fungi, Plants, and Animals as a three-axis triangular balance shape, while Bio Complexity controls the center-to-edge emerald fill from empty at 0 to fully filled at 1000. Nature also derives Bio Disbalance as the difference between the highest and lowest Fungi, Plants, and Animals values.
...
- Battle uses a wave-stepped threat counter instead of a player-facing wave counter.
- Siege threat starts at 80% of current city signature when the player switches into Battle mode. Each released siege wave advances threat one resolved `siege.threatStepPercent` step toward target threat, independent of elapsed time and independent of how quickly monsters die. The visible siege meter compares defeated monster `strengthCost` against the precomputed total wave strength budget for that siege, so it fills when monsters die while its denominator is known at siege start. During siege, the next wave appears when the resolved `siege.waveTime` countdown expires or all current wave enemies are defeated. Once the target-threat wave has been released, no new waves are generated; the siege can still be won once existing enemies are cleared, or failed later if remaining enemies overwhelm wall pressure. Leaving Battle mode resets siege progress for the next Battle entry.
- The active battlefield depth is based on the longest effective tower range plus the resolved tower `reconRange`, with only a small wall apron below the wall line. `reconRange` replaces the old fixed 3-hex visible padding beyond tower range. Enemies may define `cloakRange`, making them fade in from transparent as they move down from the battlefield top; they become tower-targetable only at full visibility, but can still be hit by projectile collision, AOE, and zone effects while cloaked. Cloaked enemies are revealed immediately when they enter the wall-distance reveal band defined by the best resolved `tower.detectionRange` among active towers and standalone wall-top defenses. Because tower stats are resolved through effective city state, city buildings, wall mounts, research, and global modifiers can all influence detection.
...
- `wallboundWobble` monster movement treats `speedPixelsPerSecond` as speed along the wobbling path, so wider wobble amplitude reduces forward progress and makes the monster take longer to reach the wall. Each spawned wobbling monster gets slight frequency jitter so identical monsters do not share one visible rhythm.
...
- If combined siege pressure exceeds combined wall resilience, the failed result splits by whether any territory can be lost. If outlying territory is lost and the resulting city signature is no longer above controlled territory, existing monsters retreat toward the left and right battlefield borders while pressure watch resumes at the new city signature. If no territory is lost, current siege threat jumps to target threat, an immediate new wave is generated, and siege progress only fully resets after the player leaves and returns to Battle.
- Failed siege signals distinguish territory loss from wall-contained failures through `siegeFailedTerritoryLost` and `siegeFailedNoTerritoryLost`, so global events can target the two outcomes separately.
...
- Tower parts can contribute single-target tower effects with `tower.singleTarget*` homogeneous values. Each effect independently selects the closest targetable enemy to the tower within that effect's range, then applies one-target push-back, flee, circle, DoT ticks, stun, or infection.
- Infection is a reusable per-enemy battle status. Applying infection adds stacks up to a max, refreshes duration, slows enemy movement by stack, and deals damage over time through the normal `DamageProfile` resolver. Wall superstructures use `tower.singleTargetInfection*` values now; future ammo can use `tower.projectileInfection*` values to apply the same status from projectile hits.
...
- Wall zone DoT damage carries runtime damage keywords from wall entities that contribute `wall.zoneDotDamage`, plus the `additionalKeywords` on those damage contributions. This lets wall superstructures such as poisonous spore mushrooms mark their DoT as `damage.poison`, `damage.antiAir`, `damage.armorPiercing`, or `damage.ignoreArmor`.
- Damage-area battle VFX are selected from `DamageProfile.keywords` through `src/data/battleDamageAreaVfx.ts`. The first mapping renders `damage.poison` DoT areas with the toxic cloud texture in `src/assets/battle/effects/toxic-cloud.png`; see `docs/damage-area-vfx.md` for the scaling model.
...
- `/monster-edit/:monsterId` edits monster entries under `src/data/enemies` and saves paired enemy sprite metadata under `src/assets/enemies/<region>`. Monster definitions may set `minimumCityVisibilityThreshold`; the battle wave planner only includes monsters whose threshold is at or below the current city visibility/threat value. Monster definitions may also set `cloakRange` and `attackMovement`; omitted attack movement defaults to standing still.
