...
- City hex background textures live in `src/assets/hexBackgrounds/<type>/<biome>/<vector>` and are cataloged through `src/data/cityHexBackgrounds.ts`. City state stores a biome, maximum city size, generated terrain vector map, per-hex current background sprite id/vector, and a per-hex base claimed-terrain sprite id/vector. Migration rerolls the biome and terrain map, initial and cleared cells use their base `claimedTerrain`, built cells may replace the visible background with `buildingUnderlay`, and the top expansion preview sector renders base claimed-terrain even while the cells remain unclaimed. Battle terrain is generated from the same hidden terrain vector map and reuses known base terrain sprite ids, extending it upward beyond the wall when needed and rendering only `claimedTerrain` hex sprites so the battlefield reads as a seamless continuation outside the city.
